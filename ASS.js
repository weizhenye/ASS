function ASS() {
  ...
  this.createTextShadow = function(oc, ow, sc, sw) {
    var ts = '';
    ow = parseFloat(ow);
    sw = parseFloat(sw);
  ...
  for(var i = 0; i < content.length; ++i) {
    ...
    if (state == 2) {
      ...
      if (/^Style:/.test(content[i])) {
        ...
        tmp2.PrimaryColour = this.toRGBA(tmp2.PrimaryColour);
        tmp2.SecondaryColour = this.toRGBA(tmp2.SecondaryColour);
        tmp2.OutlineColour = this.toRGBA(tmp2.OutlineColour);
        tmp2.BackColour = this.toRGBA(tmp2.BackColour);
...
ASS.prototype.launch = function(data) {
  var dia = document.createElement('div');
  this.set(dia, data);
  this.runline.push(dia);
};
ASS.prototype.set = function(dia, data) {
  ...
  if (s.BorderStyle == 1) dia.style.textShadow = this.createTextShadow(s.OutlineColour, s.Outline, s.BackColour, s.Shadow);
  ...
  for(var i = 0; i < t.length; ++i) {
    ...
    for(var j = 0; j < cmds.length; ++j) {
      ...
      if (/^r/.test(cmds[j])) {
        ...
        if (ss.BorderStyle == 1) diaChild.style.textShadow = this.createTextShadow(ss.OutlineColour, ss.Outline, ss.BackColour, ss.Shadow);
      }
    ...
    if (diaChild.PrimaryColour) diaChild.style.color = this.toRGBA('&H' + (diaChild.alpha[1] || '00') + diaChild.PrimaryColour);
    else if (diaChild.alpha[0]) diaChild.style.opacity = 1 - parseInt(diaChild.alpha[0], 16) / 255;
    if (diaChild.OutlineColour) diaChild.OutlineColour = this.toRGBA('&H' + (diaChild.alpha[3] || '00') + diaChild.OutlineColour);
    if (diaChild.BackColour) diaChild.BackColour = this.toRGBA('&H' + (diaChild.alpha[4] || '00') + diaChild.BackColour);
    if (diaChild.OutlineColour || diaChild.Outline != undefined || diaChild.BackColour || diaChild.Shadow != undefined) {
      ...
      diaChild.style.textShadow = this.createTextShadow(diaChild.OutlineColour, diaChild.Outline, diaChild.BackColour, diaChild.Shadow);
    }
  }
...
ASS.prototype.getChannel = function(dia) {
  var that = this,
      L = dia.Layer,
...
ASS.prototype.freeChannel = function(dia) {
